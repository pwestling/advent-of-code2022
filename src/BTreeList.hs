...
maybeSplit :: BTreeList a -> BTreeList a
...
insert index value (BTreeBranch s xs) = BTreeBranch (s + 1) (result) where
  next :: (Show a) => Int -> Int -> [BTreeList a] -> (Int, Int, BTreeList a)
...
remove index (BTreeLeaf s xs) = BTreeLeaf (s - 1) (take index xs ++ drop (index + 1) xs)
